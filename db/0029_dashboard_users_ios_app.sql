-- 2026-09-09: the users table swaps its Plan column for "iOS app" — has this
-- signed-up user ever signed in from the iPhone app?
--
-- The signal is auth.sessions.user_agent: the app's URLSession identifies as
-- "Find%20A%20Crib/<build> CFNetwork/… Darwin/…" (the web is always Mozilla/…).
-- Sessions expire and get deleted, so the sighting is persisted into
-- public.ios_app_users; dashboard_users() refreshes that table from the live
-- sessions on every call (this project has no pg_cron) and then reads it, so a
-- user who used the app once stays counted after their session is gone.

create table if not exists public.ios_app_users (
  user_id    uuid primary key references auth.users(id) on delete cascade,
  first_seen timestamptz not null,
  last_seen  timestamptz not null,
  last_build text
);
revoke all on public.ios_app_users from anon, authenticated;
grant select on public.ios_app_users to service_role;

create or replace function public.refresh_ios_app_users()
returns void
language sql
security definer
set search_path to 'public'
as $$
  insert into public.ios_app_users (user_id, first_seen, last_seen, last_build)
  select s.user_id,
         min(s.created_at),
         max(coalesce(s.refreshed_at, s.updated_at, s.created_at)),
         (array_agg(substring(s.user_agent from 'Crib/([0-9]+)') order by coalesce(s.refreshed_at, s.updated_at, s.created_at) desc))[1]
    from auth.sessions s
   where s.user_agent like 'Find%20A%20Crib/%' or s.user_agent like 'Find A Crib/%'
   group by s.user_id
  on conflict (user_id) do update
    set first_seen = least(ios_app_users.first_seen, excluded.first_seen),
        last_seen  = greatest(ios_app_users.last_seen, excluded.last_seen),
        last_build = coalesce(excluded.last_build, ios_app_users.last_build);
$$;
revoke execute on function public.refresh_ios_app_users() from public, anon, authenticated;
grant execute on function public.refresh_ios_app_users() to service_role;

-- dashboard_users: the live definition (pg_get_functiondef, 2026-09-09) plus the
-- iOS-app fields, wrapped so every call refreshes ios_app_users first.
