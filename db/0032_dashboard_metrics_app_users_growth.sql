-- dashboard_metrics: count iPhone-app users, filter unreleased builds, and add
-- the fixed-window growth figures the investor card reads (2026-09-21).
--
-- Before this the app wrote only to events, so its users were in no visitor,
-- DAU/MAU, retention or activation number. The function now takes the list of
-- builds that reached the App Store (api_server passes it from appstore.json)
-- so simulator, TestFlight and App Review launches are not users.
--
-- The signature changed, so the old one is dropped first (PostgREST cannot
-- choose between two overloads). Grants are re-issued below.
--
-- Applied to production 2026-09-21 through the Management API SQL endpoint.
drop function if exists public.dashboard_metrics(text);
create or replace function public.dashboard_metrics(p_range text default 'all', p_ios_builds text[] default null)
 returns jsonb
 language sql
 security definer
...
revoke all on function public.dashboard_metrics(text, text[]) from public, anon, authenticated;
grant execute on function public.dashboard_metrics(text, text[]) to service_role;

-- dashboard_users: a 'founding' grant is comped, not free.
