-- The Users page read 25 iPhone-app users as "Desktop": the app's sign-in
-- user agent is "Find%20A%20Crib/<build> CFNetwork/... Darwin/..." and carries
-- no "iPhone", so the device CASE fell through to its else branch.
...
select coalesce(jsonb_agg(u order by u.created_at desc), '[]'::jsonb)
  from (
    select
      ...
      (select case
                -- The iPhone app signs in as "Find%20A%20Crib/79 CFNetwork/..."
                -- — no "iPhone" in it, so 25 app users were reading as Desktop
                -- on the Users page (owner, 2026-09-23).
