...
SEEDS = [
    ...
    dict(slug="city_seo_expansion", status="candidate", kind="content",
         ...
         notes="Generated by build_seo.py:city_hub_pages() at /<city>/neighborhood/<slug>/ (SF, "
               "DC) and /la/zip/<zip>/, plus a browse hub at /<city>/buildings/, each in its own "
               "sitemap shard. Grouped on the dimension each city's data actually carries: SF and "
               "DC record a neighborhood, LA records only a ZIP. No page states a stat its city "
               "does not hold — LA has no reported rents, DC has no build years — and no place "
               "with fewer than 5 records gets a page. Deliberately NOT building-level: SF is "
               "anonymized to the block and LA is a derived 'likely RSO' list, so the aggregate "
               "is the honest unit of publication for those two."),
