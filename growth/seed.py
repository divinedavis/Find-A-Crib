...
SEEDS = [
    ...
    dict(slug="account_lifecycle", status="candidate", kind="lifecycle",
         ...
         notes="Accounts created before the 2026-07-27 cutover never enter the sequence — "
               ...
               "2026-09-25 revisit: CHANGE. The hypothesis is answered on its own terms and "
               "is no longer the interesting question — the sequence runs at real volume. "
               "Per-step sends 09-12..09-23 total ~216 emails in 16 days (welcome 3-10/day, "
               "activate 67 over 12 days, lapsed ~1/day, saved 11 in total) against "
               "accounts_with_saves 21 -> 30. Not dormant, not unmeasured. What the volume "
               "exposed is why this is CHANGE: ~6 welcome emails a day means ~100 new "
               "accounts in 16 days, and every one read a first paragraph claiming the site "
               "'maps every building registered rent-stabilized with DHCR — all 47,000 of "
               "them across the five boroughs, plus San Francisco, Los Angeles and "
               "Washington DC'. That hangs three cities off a DHCR claim true in one. LA "
               "publishes no building-level list, so that layer is derived from assessor "
               "records and labelled likely RSO; SF is published anonymized to the block, "
               "not the address. `activate` — the highest-volume step — was worse: it told "
               "the reader 'before signing a lease, search the exact address. If it comes "
               "up, the building is registered rent-stabilized', resting a lease decision "
               "on a lookup that does not mean that in two of four cities. Every published "
