-- Add a 30-day RETURN rate to dashboard_metrics(), so the returning-visitor and
-- sign-up tiles can be compared to published benchmarks on a matching basis.
--
-- The dashboard already had a `retention` block, and it answers a different
-- question than the one industry benchmarks answer. `retention.dN.rate` is a
-- SURVIVAL rate -- of visitors first seen at least N days ago, how many were
-- still turning up on or after day N. The published figure ("13% of visitors
-- return within 30 days after their initial visit", Contentsquare 2026) is a
-- RETURN rate -- how many came back at all inside the window. On this data the
-- two are far apart: d30 survival is 0.5%, and reporting that against 13% would
-- have declared the site catastrophically behind on a comparison that was never
-- valid. `within_rate` is the like-for-like figure and is what the tile shows.
--
-- The tile's own headline cannot carry the comparison either. It counts
-- returning visitors inside the SELECTED RANGE, so it is both range-dependent
-- and right-censored -- somebody first seen yesterday cannot have come back yet,
-- but is in the denominator. `within_rate` fixes both: the window is always 30
-- days regardless of the range picker, and the cohort excludes anyone who has
-- not had a full 30 days to return.
--
-- A return is a DIFFERENT DAY, built on `active_days`, matching the rule the
-- ranged `pv.days` figures have used since the channel-donut fix. A second page
-- load in the same session is not a return.
--
-- This file carries the WHOLE function, generated from pg_get_functiondef()
-- against production and then edited, per the convention 20260815140000 set --
-- applying it top-to-bottom reproduces what is live rather than reverting
-- features that exist in no migration.
--
-- Applied to production 2026-08-17. The two added scalar subqueries are one
-- extra pass over `active_days` per bucket; runtime is unchanged at ~0.4s.
--
-- Secrets review: no credentials or PII -- aggregate counts only.
-- OWASP LLM review: N/A, no LLM surface.
