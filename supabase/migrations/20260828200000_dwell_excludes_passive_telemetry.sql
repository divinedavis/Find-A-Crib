-- dashboard_metrics(): passive telemetry must not count as a session hit.
--
-- The `hits` CTE that sessions are built from was `visits UNION ALL events`,
-- every event included. Two of those events are not things a visitor does:
-- `tile_impression` fires when an advertiser tile scrolls into view, and
-- `tile_served` (shipped 2026-08-28) fires when one is merely RENDERED —
-- roughly 1,500 times a day, with no interaction required.
--
-- Counting them promoted bounces into "engaged sessions" lasting a couple of
-- seconds each. Measured on 2026-08-28: today's engaged-session median read
-- 29s against 104s with those two events removed, on 62 "engaged" sessions
-- against a real 37 — and the card then judged 29s "below average" against a
-- 3m 52s industry benchmark, which is a false negative produced entirely by
-- our own instrumentation.
--
-- This file is the FULL live function as it stands after the fix, dumped with
-- pg_get_functiondef and patched, because the live definition had already
