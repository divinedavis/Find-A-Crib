-- Savers who came back within 30 days — the return rate a listings portal
-- optimises for.
--
-- Zillow reports ~239M monthly uniques and ~2.5B visits a quarter (Q2 2026),
-- i.e. ~3.5 visits per user per month, and those visits are driven by
-- saved-search alerts rather than habit. The dashboard's goals were re-cut
-- on 2026-09-03 around that shape: WAU/MAU (already in `engagement`) as the
-- stickiness that matters, DAU/MAU 10% as the category ceiling, and THIS —
-- of accounts whose first save (building or search) is at least 30 days old,
-- how many had an active day within 30 days after it. Fixed window, the range
-- picker does not apply. Owner excluded by user_id. Null rate on an empty
-- cohort, not 0.
--
...
-- 20260901120000 before this edit (pg_get_functiondef against the live
-- database), so this is that definition plus `saver_first`/`user_days`/
-- `saver_ret` and the `saver_return` key.
