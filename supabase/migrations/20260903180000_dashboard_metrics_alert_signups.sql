-- Borough-alert sign-ups on the owner dashboard ("Product health").
--
-- /alerts/ shipped 2026-09-03: a free, no-account email list — "the minute a
-- new housing lottery or re-rental opens in <borough>". It is the first
-- surface on the site that captures intent without an account, so it belongs
-- with the numbers that value the product, not just in a table nobody reads.
--
-- `alerts` key: active subscribers (all time), sign-ups inside the picked
-- range, unsubscribes, how many set a rent/income filter, the borough split,
-- and emails actually sent. The owner's own subscription is excluded by
-- matching the owner's auth.users email — the uuid this function already
-- uses for the paying count — so no address is written into a public repo.
-- The tile's lesson is Crease's: the owner's testing must never be the whole
-- demand number.
--
-- Whole function replaced because it is a LANGUAGE sql function. Verified
-- byte-identical to 20260903120000 before this edit (pg_get_functiondef
-- against the live database), so this is that definition plus the `alerts`
-- key.
...
with
  ...
  user_days as (
    ...
    from v_all where user_id is not null
  ),
  saver_ret as (
    select
