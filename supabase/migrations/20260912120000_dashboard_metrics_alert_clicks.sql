-- Alert-email clicks + alert sign-up conversion on the owner dashboard.
--
-- Adds click counts from public.alert_clicks (db/0030) to the `alerts` key:
-- distinct subscribers who clicked, non-bot clicks, scanner hits, split by
-- email kind, all following the range picker; plus `emailed` (subscribers who
-- got at least the welcome) as the click-rate denominator. The alert sign-up
-- conversion tile needs no SQL: alerts.signups over totals.visitors, both
-- already ranged.
--
-- Built from pg_get_functiondef() of the LIVE function on 2026-09-12, not from
-- the previous migration file (see reference_findacrib_sql_function_drift).
