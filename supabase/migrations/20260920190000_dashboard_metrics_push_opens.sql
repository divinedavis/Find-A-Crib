-- The alert-click tile counted only clicks on links in alert emails; a push
-- notification tapped in the iPhone app is the same act and was invisible
-- (owner, 2026-09-20: "alert clicks should count iphone notifications too").
-- Adds push_opens / push_openers to the `alerts` key; the dashboard adds them
-- to the email numbers and shows the split underneath.
--
-- Taken from pg_get_functiondef on the live function, because the live
-- definition drifts ahead of these files (see reference_findacrib_sql_function_drift).
...
select jsonb_build_object(
  ...
  'alerts', (
    ...
    select jsonb_build_object(
      ...
      'push_opens',   (select count(*) from public.events e, bounds
                        where e.event = 'push_open' and e.created_at >= bounds.since),
      'push_openers', (select count(distinct coalesce(e.user_id::text, e.visitor_id)) from public.events e, bounds
                        where e.event = 'push_open' and e.created_at >= bounds.since),
