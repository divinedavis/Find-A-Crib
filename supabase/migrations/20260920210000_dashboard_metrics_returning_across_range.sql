-- "Returning visitors" counted only days inside the selected range, so the
-- Today tile read 0 · 0% every day and This month missed anyone whose earlier
-- visit was last month (owner, 2026-09-20). Returning now also means "seen on
-- any day before the range started". Live definition via pg_get_functiondef,
-- applied through the Management API.
