-- dashboard_metrics: skip the passive ad-tile rows (see the ev_all comment).
-- This is the full LIVE definition as of 2026-09-25, pulled with
-- pg_get_functiondef: the p_ios_builds version was applied without a
-- migration, so the last file in this folder (20260920220000) is not what ran.
...
with
  ...
  -- The two passive ad-tile events are ~60% of the table (tile_served alone
  -- 152,763 rows on 2026-09-25, ~4,000 a day) and nothing below counts them:
  -- the session and funnel CTEs already exclude them and the ad-tile card has
  -- its own function (dashboard_adtiles). Reading them anyway pushed a cold
  -- 3-month call past api_server's 8 s RPC timeout, so the range picker sat
  -- disabled and then failed. Output is identical except that the 'events'
  -- count map no longer lists these two keys (the page never reads them).
  -- 3m 3.09 s -> 1.75 s, all 2.64 s -> 1.77 s.
  ev_all as (
