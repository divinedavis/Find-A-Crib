-- The Advertiser metrics card now measures every ad, not one kind of tile
-- (owner, 2026-09-25): reach_all = distinct people shown any ad (our tiles or
-- a live Google ad), google_clicks = live AdMob taps. Everything else is
-- pg_get_functiondef() of the live function (20260926010000), unchanged.
...
select jsonb_build_object(
  ...
  -- The card measures ads, not one kind of tile (owner, 2026-09-25): people
  -- shown ANY ad, and Google's own clicks (AdMob reports app taps; AdSense
  -- clicks happen inside Google's iframe and only its report sees them).
  'reach_all',        (select count(distinct v) from (
                         select nullif(visitor_id, '') v from r
                         union
                         select nullif(visitor_id, '') from public.events
                          where event = 'ad_impression' and coalesce(props->>'mode', '') = 'live'
                            and (p_since is null or created_at >= p_since)
                            and (visitor_id is null or visitor_id <> all (coalesce(p_exclude, '{}')))) x
                       where v is not null),
  'google_clicks',    (select count(*) from public.events
                        where event = 'ad_click' and coalesce(props->>'mode', '') = 'live'
                          and (p_since is null or created_at >= p_since)
                          and (visitor_id is null or visitor_id <> all (coalesce(p_exclude, '{}')))),
